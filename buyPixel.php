...
//
$addedCoords = $firestore->collection('coords')->document('added-coords');
$coords = $addedCoords->collection('savedcoords')->documents();
//
if($canUploadPhoto === true) {
	foreach ($coords as $coord) {
//	echo $coord->exists();
		if ($coord->id() === $grab_width . "," . $grab_height) {
			header("Location: buyPixel.html");
			//$coord['x'] === $grab_width && $coord['y'] === $grab_height
			// &&
		} else {
			$firestore->collection('coords')->document('request-coords')->
			collection('askedcoords')->document($grab_width . "," . $grab_height)->set([
				'x' => $grab_width,
				'y' => $grab_height,
				'time' => time()
			]);
			$bucket = $storage->bucket('req-memes');
